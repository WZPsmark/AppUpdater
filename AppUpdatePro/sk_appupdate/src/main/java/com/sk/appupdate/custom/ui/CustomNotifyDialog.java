...
public class CustomNotifyDialog extends BaseUpdateDialog {
    ...
    protected void bindView(View view) {
        ...
        CircleImageView app_icon = view.findViewById(R.id.app_icon);

        title.setText(String.format("%s需要更新啦!", AppUtil.getAppName(getContext())));
        app_icon.setImageDrawable(AppUtil.getDrawable(getContext()));
        if (mUpdateInfoBean != null) {
            new_version_name.setText(String.format("最新版本：V%s", mUpdateInfoBean.getNew_version()));
            size.setText(String.format("新版本大小：%s", mUpdateInfoBean.getTarget_size()));
